...
############################################
# EPD (1 Encoder + 3 PD consumers + Proxy)
############################################
run_epd_mooncake() {
  cleanup

  echo "[EPD] Mooncake Starting Encoder on GPU $GPU_E (port $ME_PORT) ..."
  ...
  vllm serve "$MODEL" \
    ...
    --ec-transfer-config "{
        ...
        \"ec_connector_extra_config\": {
            ...
            \"transfer_buffer_size\": \"16106127360\"
        }
    }" \
    > "${LOG_PATH}/epd_mooncake_encoder.log" 2>&1 &

  echo "[EPD] Starting PD consumer #1 on GPU $GPU_PD (port $MPD_PORT) ..."
  ...
  vllm serve "$MODEL" \
    ...
    --ec-transfer-config "{
        ...
        \"ec_connector_extra_config\": {
            \"protocol\": \"rdma\",
            \"device_name\": \"mlx5_2,mlx5_3\",
            \"transfer_buffer_size\": \"2684354560\"
        }
    }" \
    > "${LOG_PATH}/epd_mooncake_pd1.log" 2>&1 &

  echo "[EPD] Starting PD consumer #2 on GPU $GPU_PD2 (port $MPD2_PORT) ..."
  CUDA_VISIBLE_DEVICES="$GPU_PD2" \
  vllm serve "$MODEL" \
    --gpu-memory-utilization 0.8 \
    --trust-remote-code \
    --port "$MPD2_PORT" \
    ...
    > "${LOG_PATH}/epd_mooncake_pd2.log" 2>&1 &

  echo "[EPD] Starting PD consumer #3 on GPU $GPU_PD3 (port $MPD3_PORT) ..."
  CUDA_VISIBLE_DEVICES="$GPU_PD3" \
  vllm serve "$MODEL" \
    --gpu-memory-utilization 0.8 \
    --trust-remote-code \
    --port "$MPD3_PORT" \
    ...
    > "${LOG_PATH}/epd_mooncake_pd3.log" 2>&1 &

  wait_port "$ME_PORT" "Mencoder" "epd_encoder.log"

  wait_port "$MPD_PORT" "Mpd1" "epd_mooncake_pd1.log"
  wait_port "$MPD2_PORT" "Mpd2" "epd_mooncake_pd2.log"
  wait_port "$MPD3_PORT" "Mpd3" "epd_mooncake_pd3.log"

  echo "[EPD] Starting Proxy on port $MPROXY_PORT ..."
  if [[ ! -f "disagg_epd_proxy.py" ]]; then
    echo "[ERROR] disagg_epd_proxy.py not found in current directory."
    echo "        Put it here or run from vLLM repo:"
    echo "        examples/online_serving/disaggregated_encoder/disagg_epd_proxy.py"
    exit 1
  fi
  ...
  python disagg_epd_proxy.py \
    --host 0.0.0.0 \
    --port "$MPROXY_PORT" \
    --encode-servers-urls "http://localhost:$ME_PORT" \
    --prefill-servers-urls "disable" \
    --decode-servers-urls "http://localhost:$MPD_PORT,http://localhost:$MPD2_PORT,http://localhost:$MPD3_PORT" \
    > "${LOG_PATH}/epd_mooncake_proxy.log" 2>&1 &

  wait_port "$MPROXY_PORT" "Mproxy" "epd_mooncake_proxy.log"
  run_bench "$MPROXY_PORT" "epdm"
  echo "[EPD] Finished."
}
